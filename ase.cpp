...
static unsigned int g_Texture[MAX_TEXTURES] = {0};
static unsigned int texture_number = 0;
#endif //LOAD_TEXTURES
...
bool CLoadASE::ImportASE(t3DModel *pModel, char *strFileName)
{
    ...
    for(int i = 0; i < pModel->numOfMaterials; i++)
    {
        ...
        if(strlen(pModel->pMaterials[i].strFile) > 0)
        {
            ...
            CreateTexture(g_Texture, pModel->pMaterials[i].strFile, texture_number);           
        }

        // Set the texture ID for this material
        pModel->pMaterials[i].texureId = texture_number;
		texture_number++;
    }
#endif //LOAD_TEXTURES
